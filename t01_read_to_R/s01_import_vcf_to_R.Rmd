...
# VCF file name
vcf_file <- file.path(project_folder,"data","s05_annotate","CCLG_GL_hg38.bwa_10bp_vqsr_qual_dp_sma_cln_tag_ac_id_clinvar_vep_splrm.vcf.gz")

# Read vcf to vcfR object 
# called "full.vcfr"" to distinguish with the possible later subsetted versions of vcfr 
...
dim(full_gt.mx)
full_gt.mx[1:5,1:3]
sum(is.na(full_gt.mx))/(ncol(full_gt.mx)*nrow(full_gt.mx))
...
hist(full_dp.mx) 
hist(log10(full_dp.mx))
sum(full_dp.mx < 10, na.rm=T)/(nrow(full_dp.mx)*ncol(full_dp.mx)) # fraction of genotypes to be removed
...
hist(full_gq.mx)
sum(full_gq.mx < 20, na.rm=T)/(nrow(full_gq.mx)*ncol(full_gq.mx)) # fraction of genotypes to be removed
